...
function crearDBConnnection() {
    con = mysql.createConnection({host: "dam.inspedralbes.cat", user: "a22tomybanog_Projecte1", password: "Projecte1", database: "a22tomybanog_Projecte1"});
...
function crearProducte(imatgeNom, nom, definicio, preu, categoria, quantitat) {
    ...
    con.query("INSERT INTO Producte SET ?", nouProducte, (error, results) => {
        if (error) {
            ...
            console.log("Producte insertado con éxito. Nom del Producte:", nouProducte.nom);
        }
    });
}
...
function deleteProducte(idProducteEliminar) {
    con.query("DELETE FROM Producte WHERE id = ?", idProducteEliminar, (error, results) => {
        if (error) {
            console.error("Error al insertar Producte:", error);
        } else {
...
function updateProducte(idProducteUpdate, imatgeNom, nom, definicio, preu, categoria, quantitat) {
    const update = {
        imatgeNom: imatgeNom,
        nom: nom,
        definicio: definicio,
    ...
    con.query(`UPDATE Producte SET ? WHERE id = ?`, [
        update, id
    ], (error, results) => {
        if (error) {
...
function addCategoria(nomCategoria) {
    con.query("INSERT INTO Categoria SET nom=?", nomCategoria, (error, results) => {
        if (error) {
            ...
            console.log("Categoria insertada con éxito. Nom de la Categoria:", nomCategoria);
        }
    });
}
...
function deleteCategoria(idCategoria) {
    con.query("DELETE FROM Categoria WHERE id=?", idCategoria, (error, results) => {
        if (error) {
            ...
            console.log("Categoria vorrada con éxito. ID de la Categoria:", idCategoria);
        }
    });
}
...
function updateCategoria(idCategoria, nomCategoria) {
    con.query("UPDATE Categoria SET nom=? WHERE id=?", [
...
function crearCarritoProducte(quantitat, idCarrito, idProducto) {
    ...
    con.query("INSERT INTO Carret_Productes SET ?", nouCarritoProducte, (error, results) => {
        if (error) {
...
function deleteCarritoProducto(idCarritoProductoEliminar) {
    con.query("DELETE FROM Carret_Productes WHERE id_carret_producte = ?", idCarritoProductoEliminar, (error, results) => {
        if (error) {
            console.error("Error al borrar carrito_producto:", error);
        } else {
            console.log("carrito Producto eliminado con éxito. ID del carrito Producto:", idCarritoProductoEliminar);
        }
    });
}
...
function deleteComandes(id_Comanda) {
    con.query("DELETE FROM Comanda WHERE id = ?", [id_Comanda], (error, results) => {
        if (error) {
            console.error("Error al borrar comanda:", error);
        } else {
            console.log("Comanda eliminada con éxito. ID de la comanda:", id_Comanda);
        }
    });
}
// function update comandes                                     (comprobada)
function updateComandes(id_comanda, finalitzat) {
    con.query("UPDATE Comanda SET finalitzat = ? WHERE id = ?", [
...
app.get("/api/validateLogin", async (req, res) => {
    ...
    con.query("SELECT * FROM Usuaris", (error, results, fields) => {
        ...
        if (usuarioEncontrado) { // Si el usuario y la contraseña coinciden, devuelve un mensaje de éxito o los datos relevantes
            return res.status(200).json({Boolean: true});
        } else { // Si el usuario y la contraseña no coinciden, devuelve un mensaje de error
...
app.post("/api/addProduct", imatges.single("img"), async (req, res) => {
    ...
    fs.rename(`./img/productes/${
        req.file.filename
    }`, `./img/productes/${newFileName}`, async function () {
...
app.post("/api/updateProduct", async (req, res) => {
    ...
    await crearDBConnnection();
    await updateProducte(idProducteUpdate, imatgeNom, nom, definicio, preu, categoria, quantitat);
    closeDBconnection();
...
// Ruta guardar imatges                                         (comprobada)
...
app.get("/api/getImage/:img", (req, res) => {
    res.sendFile(path.resolve(`./img/productes/${
        req.params.img
    }`));
});
